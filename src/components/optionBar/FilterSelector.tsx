...
const FilterSelector: React.FC<IPros> = () => {
  ...
  const isIncaseSearch = (obj: any) => {
    for (const prop in obj) {
      if (
        prop.toLowerCase() === col.toLowerCase() &&
        obj[prop] === value &&
        oper === "equals"
      )
        return true;
  ...
  const colDropDownClickHandler: React.MouseEventHandler<HTMLLIElement> = (
    e
  ) => {
    e.preventDefault();
  ...
  const operDropDownClickHandler: React.MouseEventHandler<HTMLLIElement> = (
    e
  ) => {
    e.preventDefault();
