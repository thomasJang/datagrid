...
import { useDatagridLayoutContext, useDatagridLayoutDispatch } from "../context/DatagridLayoutContext";
...
const DatagridBody: React.FC<IDatagridBody> = (props) => {
  ...
  const { _bodyHeight = 1, _bodyWidth = 1, _scrollTop, _scrollLeft } = layoutContext;
  ...
  const { startRowIndex, endRowIndex, styleTop } = React.useMemo(() => {
    ...
    const endRowIndex = startRowIndex + displayRowCount > dataLength ? dataLength : startRowIndex + displayRowCount;
  ...
  return (
    <div ref={containerRef} className="ac-datagrid--body" style={props.style}>
      <BodyAsidePanel startRowIndex={startRowIndex} endRowIndex={endRowIndex} styleTop={styleTop} />
      <BodyLeftPanel />
      <BodyMainPanel startRowIndex={startRowIndex} endRowIndex={endRowIndex} styleTop={styleTop} styleLeft={styleLeft} />
      {props.children}
    </div>
  );
};
